        ...
        for file in files:
            if file.endswith(".txt"):
                ...
                for i, line in enumerate(lines):
                    data = line.split()
                    if len(data) >= 5:
                        measure, latitude, longitude, altitude, satellite = data[0], data[1], data[2], data[3], data[4]
                        name = f"Medida_{i + 1}"
                        kml_body += kml_placemark_template.format(name=name, measure=measure, longitude=longitude, latitude=latitude, altitude=altitude, satellite=satellite)
